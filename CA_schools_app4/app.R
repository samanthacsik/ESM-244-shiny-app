    ...
    tabsetPanel(
      ...
      tabPanel("Population, Income & Enrollment",
               fluidRow(
                 ...
                 selectInput("county", "Select County", county_names),
                 dataTableOutput(outputId = 'selectedCounty')
                 
                 # selectInput("district", "Select District", district_names),
                 # dataTableOutput(outputID = 'selectedDistrict')
               )),
...
shinyServer(function(input, output) {
  ...
  output$countySelection = renderDataTable({
    subset(county_counties, NAME=input$county)
    
  })
  
  # output$districtSelection = renderDataTable({
  #   subset(something, DISTRICT=input$district)
  # })
  # 
})
